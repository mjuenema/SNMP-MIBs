...
for inputMib in progressbar.progressbar(inputMibs):
    ...
    try:
        json_compiler = MibCompiler(
            SmiStarParser(), JsonCodeGen(), CallbackWriter(writer)
        )

        # search for source MIBs here
        json_compiler.addSources(*[FileReader(x) for x in srcDirectories])

        # search for source MIBs at Web sites
        #json_compiler.addSources(*[HttpReader(*x) for x in httpSources])

        # never recompile MIBs with MACROs
        json_compiler.addSearchers(StubSearcher(*JsonCodeGen.baseMibs))

        # run recursive MIB compilation
        json_compiler.compile(inputMib)
    except KeyError as e:
        print(inputMib, e)


with open('miboid.json', 'w') as fp:
